  ...
  <div v-if="source" class="m-item" :class="{'m-item-equipment':source.AucGenre>=1&&source.AucGenre<=4}">
    <div class="m-item-wrapper">
      ...
      <div v-if="parseInt(source.MaxExistTime) > 0" class="u-max-exist-time"
           v-text="'存在时间：' + second_format(source.MaxExistTime)"></div>
      <!-- 最大拥有数 -->
      ...
      <div class="m-attributes" v-if="source.attributes&&source.attributes.length">
        <div v-for="(attribute,key) in source.attributes" :key="key" class="m-field" :class="[`u-${attribute.color}`]">
          ...
          <span v-else-if="attribute.type=='atHorseAttribute'"
            ...
            <img v-if="attribute.icon_id>0" class="u-horse-icon" :src="icon_url(attribute.icon_id)">
            <div class="u-horse-desc" v-html="attribute.label"></div>
          </span>
          <span v-else class="u-value">
            <span v-text="attribute.label"></span>
            <span class="u-yellow" v-text="attribute_percent(attribute.type, attribute.value)"></span>
          </span>
        </div>
      </div>
      <!-- 镶嵌 -->
...
import icon_url from "../icon_url.js";
import second_format from "../second_format.js";
import attribute_percent from "../module/item/attribute_percent.js";
import bind from "../module/item/bind.js";
import color from "../module/item/color.js";
...
@import '../../css/components/item.less';
</style>
