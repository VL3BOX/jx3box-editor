<template>
    <div class="c-editor-markdown-dialog">
        <span class="c-markdown-toolbar-file c-markdown-toolbar-item" @click="handleClick" :title="$t('插入奇穴方案')">
            {{ $t('奇') }}
        </span>

        <el-dialog :visible.sync="visible" :title="$t('插入奇穴方案')" :modal-append-to-body="false">
            <p>💙 {{ $t('请填入') }}<a target="_blank" href="/app/talent">{{ $t('奇穴模拟器') }}</a>{{ $t('生成的编码') }}</p>
